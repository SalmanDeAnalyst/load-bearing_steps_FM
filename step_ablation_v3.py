...
MINI_BATCH = 50               # CFG doubles to 200, fine for 16GB
...
CHECKPOINT_PATH = "/content/best_model_ema.pt"
OUTPUT_DIR = "/content/ablation_results_v3"
...
class FIDMetric:
    ...
    def compute_fid(feats1, feats2):
        ...
        sqrt_product = np.real(sqrtm(sigma1 @ sigma2))
...
# =========================================================
# VISUAL ABLATION — 3 samples × 10 steps × 3 perturbations
# =========================================================
@torch.no_grad()
def visual_ablation(model):
    print("\n" + "=" * 70)
    print("VISUAL ABLATION: 3 samples × 10 steps × 3 perturbation types")
    print("=" * 70)

    VISUAL_NFE = 5
    torch.manual_seed(42)

    # Generate 10 images, pick 3
    n = 10
    labels = torch.arange(n, device=DEVICE) % NUM_CLASSES
    x0 = torch.randn(n, CHANNELS, IMAGE_SIZE, IMAGE_SIZE, device=DEVICE)
    swap = generate_swap_labels(labels)

    # Generate baseline to pick from
    baseline = euler_sample_with_perturbation(model, x0, labels)
    sample_indices = [0, 4, 8]
    perturb_types = ["zero", "gaussian", "class_swap"]
    perturb_labels = {"zero": "Zero-Out", "gaussian": "Gaussian", "class_swap": "Class Swap"}

    for s_num, s_idx in enumerate(sample_indices):
        x0_s     = x0[s_idx:s_idx+1]
        label_s  = labels[s_idx:s_idx+1]
        swap_s   = swap[s_idx:s_idx+1]
        cls_name = CIFAR10_CLASSES[label_s[0].item()]

        fig, axes = plt.subplots(3, VISUAL_NFE, figsize=(VISUAL_NFE * 1.8, 6))
        fig.suptitle(f"Sample {s_num+1} | class='{cls_name}'", fontsize=11, fontweight="bold")

        for p_idx, pt in enumerate(perturb_types):
            for step_k in range(VISUAL_NFE):
                img = euler_sample_with_perturbation(
                    model, x0_s, label_s,
                    nfe=NFE,
                    perturb_step=step_k,
                    perturb_type=pt,
                ...
                img_np = np.clip((img[0].cpu().permute(1,2,0).numpy() + 1) / 2, 0, 1)

                ax = axes[p_idx][step_k]
                ax.imshow(img_np, interpolation="nearest")
                ax.axis("off")
                if p_idx == 0:
                    ax.set_title(f"k={step_k}", fontsize=7)
                if step_k == 0:
                    ax.set_ylabel(perturb_labels[pt], fontsize=8)

        plt.tight_layout()
        plt.savefig(f"/content/visual_ablation_sample{s_num+1}.png", dpi=150, bbox_inches="tight")
        plt.show()
        print(f"  Saved: visual_ablation_sample{s_num+1}.png")
...
#     # Load model for visual ablation
#     model = ClassConditionalUNet().to(DEVICE)
#     state = torch.load(CHECKPOINT_PATH, map_location=DEVICE, weights_only=True)
#     model.load_state_dict(state)
#     model.eval()
#     visual_ablation(model)
